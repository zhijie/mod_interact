...
user_send_packet({Packet, C2SState}) ->
    ...
    if (Type == groupchat) ->
    	...
    	if (Body /= <<"">>) ->
			AppKey = gen_mod:get_module_opt(From#jid.lserver, ?MODULE, app_key, fun(S) -> iolist_to_binary(S) end, list_to_binary("")),
			MasterSecret = gen_mod:get_module_opt(From#jid.lserver, ?MODULE, master_secret, fun(S) -> iolist_to_binary(S) end, list_to_binary("")),
